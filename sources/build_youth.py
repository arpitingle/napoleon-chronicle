...
out = []
RANGE = ("1789-01-01", "1821-12-31")  # the app cannot render outside this


def in_range(iso):
    return RANGE[0] <= iso <= RANGE[1]
push(out, NAP, "Corsica", "1791-01-23", None, "Ajaccio",
     ...
     "letter",
     "The 21-year-old denounces Corsica's deputy: treason itemized in letters of blood. Dated by the edition." + TR,
...
push(out, NAP, "Paris (manuscripts)", "1788-01-01", None, "Paris",
     ...
     "literary fragment",
     "Undated fragment placed by editors: the boy lends voice to Corsica's failed king — the editors link it to St Helena." + TR,
     "youth-corsica", cert="approximate")
...
kept = [r for r in out if in_range(r["date"])]
for r in out:
    if r not in kept:
        print("skipped (out of app range %s..%s): %s %s"
              % (RANGE[0], RANGE[1], r["id"], r["date"]))
json.dump(kept, open("data/nap_youth.json", "w"), ensure_ascii=False, indent=1)
print("youth:", len(kept))
